...
public:
    ...
    vectore(int n, vectore<T> &t = T()) : data(NULL), index(0), cap(1)
    {
        data = new T[cap];
        for (int i = 0; i < n; i++)
            push_back(t);
    }
    vectore(const T *first, const T *end) : data(NULL), index(0), cap(1)
    {
        data = new T[cap];
        while (first != end)
        {
    ...
    vectore<T> &assign(const vectore<T> &rhs)
    {
    ...
    vectore<T> &operator=(const vector<T> &rhs)
    {
        return assign(rhs);
    }
};
